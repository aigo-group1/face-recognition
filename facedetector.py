...
import argparse
import sys
from facial_landmarks.headpose import set_camera_matrix, head_pose_estimation
...
def detect_video(video_path,isout=True):
    ...
    set_camera_matrix(video_size[1], video_size[0])                        
    ...
    while True:
        ...
        if frame is not None:
            image, rects = getFrameBoundingBoxes(frame)
            if len(rects) == 0:
                im = image
            else:    
                im = head_pose_estimation(image, rects)
            result = im
            curr_time = timer()
